...
love.serial.pack = function(t, drop, indent)
	assert(type(t) == "table", "Can only love.serial.pack tables.")
	...
	local function proc(k,v, omitKey)	-- encode a key/value pair
		...
		elseif tk == "table" then k = "["..love.serial.pack(k, drop, indent and indent+1).."]"
		elseif type(drop) == "function" then k = "["..string.format("%q",drop(k)).."]"
		elseif drop then skip = true
		else error("Attempted to love.serial.pack a table with an invalid key: "..tostring(k))
		end
		...
		elseif tv == "table" then v = love.serial.pack(v, drop, indent and indent+1)
...
love.serial.packPretty = function(t, drop, indent, deep)
	local tab = ""
	if not deep then
		deep = 1
	end
	...
	local function proc(k,v, omitKey)	-- encode a key/value pair
		...
		elseif tk == "string" then
			...
			if f ~= '"'..k..'"' or string.find(k, " ") then k = '['..f..']' end
		elseif tk == "number" then k = "["..k.."]"
		elseif tk == "table" then k = "["..love.serial.packPretty(k, drop, indent and indent+1, deep + 1).."]"
		elseif type(drop) == "function" then k = "["..string.format("%q",drop(k)).."]"
		elseif drop then skip = true
		else error("Attempted to love.serial.pack a table with an invalid key: "..tostring(k))
		end
		...
		elseif tv == "string" then v = string.format("%q", v)
		elseif tv == "number" then	-- no change needed
		elseif tv == "table" then v = love.serial.packPretty(v, drop, indent and indent+1, deep + 1)
		elseif type(drop) == "function" then v = string.format("%q",drop(v))
		else skip = true
		end
		if not skip then
			return string.rep("\t",indent or 0)..(omitKey and "" or k.." = ")..v..",\n"..tab..(indent and "\n" or "")
		end
		return ""
	end
	local l, did=-1,{} repeat l=l+1 until t[l+1]==nil	-- #t "can" lie!
...
--- Loads a table into memory from a string (like those output by love.serial.pack)
...
love.serial.unpack = function(s, safe)
	if safe then s = string.match(s, "(%b{})") end
	assert(type(s) == "string", "Can only love.serial.unpack strings.")
	local f, result = loadstring("love.serial.table="..s)
	if not safe then assert(f,result) elseif not f then return nil, result end
	result = f()
	local t = love.serial.table
	love.serial.table = nil
	return t, result
end